...
- Fixed the GitHub Actions parse-validation step in `.github/workflows/ci.yml` so parse error messages are built safely before `throw`. The previous inline `-join` expression inside the interpolated string caused the workflow itself to fail before repository scripts were evaluated.
